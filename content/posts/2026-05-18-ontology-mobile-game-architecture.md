---
categories: ["技术Architecture", "AI 实践"]
title: "AI 时代的Architecture方法论：Ontology与手游行业实践"
date: 2026-05-18T09:30:00+08:00
lastmod: 2026-05-18T09:30:00+08:00
summary: "从 DDD 到Ontology（Ontology）的演进。解析为何 AI 进不去核心业务决策层，以及手游团队如何通过Ontology构建可审计、可治理的‘世界说明书’。"
tags: ["AI Architecture", "Ontology", "DDD", "手游研发", "Architecture设计", "Agent 治理"]
---

> 本文基于葡萄城技术团队《从领域驱动到Ontology：AI 时代的Architecture方法论变了》的解读，结合我们游戏研发 AI 协作平台的实践，给出**可落地**的手游视角Architecture方案。

## 0. 一句话先说清

**Ontology（Ontology）** 不是哲学课里的「存在论」。在软件工程里，它指的是：把**游戏/业务世界里的东西（对象）、关系、能做什么（操作）、不能做什么（约束）**，建成一套**机器和人都能查、能执行、能审计**的语义模型，并把它放在**平台层**而不是散落在代码里。
...
博文结论可概括为：**不是 DDD 错了，是没有「把模型钉在平台上」的工程载体。** Ontology方法论 + 元数据驱动平台，是把 DDD 的「约定」升级成「基础设施」。

---

## 3. Ontology是什么：用「游戏策划表」的升级版来理解
...
这与**游戏研发 AI 协作平台**中「双模运行」（人读 `.cursorrules` / Agent Server 读同一规则）方向一致：规则必须**单源、可读、可执行**；Ontology进一步要求规则**单源在语义平台**，而非仅在自然语言规则文件里。

---

## 5. 手游行业：如何用Ontology「更有创意、更有效率」
...
- **Ontology**：把对象、关系、操作、约束升为**平台元数据**，并记录决策，形成闭环。
- **对手游**：创意来自**在安全沙盒里重组对象与 Action**；效率来自**LiveOps 与研发复用同一语义层**。

**Architecture师在 AI 时代的上移**，不是少写代码，而是**先把游戏世界建模清楚**——让策划、程序、运营与 Agent 在同一套「世界说明书」上协作。
