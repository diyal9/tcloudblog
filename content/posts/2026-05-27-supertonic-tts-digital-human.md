...
| 参数量 | N/A（系统级） | 99M |
| 情感控制 | 不支持 | 支持 10 种表情标签 |
| 声音定制 | 不支持 | 支持 Voice Builder 克隆 |
| 部署 | 浏览器端 | 服务端 / 本地 |
...
1. **SupertonicTTS**：flow matching Architecture的 text-to-latent 模块
...
1. **阶段 1**：HTTP 模式验证中文质量
   ↓
2. **阶段 2**：确认效果 → Go 原生集成（消除网络延迟）
   ↓
3. **阶段 3**：表情标签 + 口型同步联动
...
- [支持] **极小模型**（99M vs 行业 700M-2B）
- [支持] **CPU 实时推理**（树莓派都能跑）
- [支持] **31 语言**（含中日韩）
- [支持] **情感标签**（数字人必备）
- [支持] **OpenAI 兼容 API**（迁移成本极低）
- [支持] **声音克隆**（专属音色）
